...
app.get("/", (req, res) => res.send("Donation app on Vercel"));
...
app.post("/stripe/create-checkout-session", async (req, res, next) => {
    ...
    form.parse(req, async (err, fields, files) => {
      try {
        ...
        res.status(500).send("Error processing Stripe session");
        // If using middleware, pass the error to the next middleware
        // next(error);
      }
    });
  });
