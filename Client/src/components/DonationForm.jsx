...
function DonationForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    setSubmit(true);
    console.log("Donation submitted: " + amount + " " + paymentType.mode + " " + paymentType.interval);
    try {
  ...
  function ButtonAmount(props) {
    const className = "block w-full p-4 bg-sky-300 text-white font-medium text-lg leading-tight uppercase rounded shadow-md hover:bg-blue-400 hover:shadow-lg focus:bg-blue-400 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-500 active:shadow-lg transition duration-150 ease-in-out";
    const classNameActive = "block w-full p-4 bg-blue-400 text-white font-medium text-lg leading-tight uppercase rounded shadow-md";
          ...
          <div className="basis-full p-2">
            ...
            {isSubmit ? (
              <button type="submit" className="block w-full p-8 bg-purple-800 text-white font-medium text-lg leading-tight uppercase rounded shadow-md" disabled>
                <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white inline-block align-top" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
